...
class CommandExecutor {
  ...
  async executeCommand(command: string, sessionId?: string): Promise<string> {
    ...
    try {
      ...
      if (command.includes('\n')) {
        ...
        const ascript = this.buildSessionScript(sessionId, `write text (${escapedCommand})`);
  ...
  private buildSessionScript(sessionId: string | undefined, action: string): string {
    if (sessionId) {
      return `
    ...
    return `tell application "iTerm2" to tell current session of current window to ${action}`;
  }

  private async retrieveTtyPath(sessionId?: string): Promise<string> {
    try {
      const ascript = this.buildSessionScript(sessionId, 'get tty');
  ...
  private async isProcessing(sessionId?: string): Promise<boolean> {
    try {
      const ascript = this.buildSessionScript(sessionId, 'get is processing');
      const { stdout } = await this._execPromise(`/usr/bin/osascript -e '${ascript}'`);
